...
/// Synthetic-ingest marker. Any swap row whose `transaction_hash`
/// equals this constant was generated by `synthetic_mock` and can be
/// safely replaced on re-ingestion. Real Ethereum tx hashes are
/// uniformly distributed 256-bit so the collision probability against
/// this all-zero-prefix tag is ~2^-224 — effectively zero.
pub const SYNTHETIC_TX_HASH: &str =
    "0x00000000000000000000000000000000000000000000000000000000deadbeef";

impl Storage {
    /// Removes any prior synthetic swap rows that overlap `[from, to]`
    /// for `pool_address`. Called by the synthetic ingest path before
    /// inserting fresh rows, so a tweak to the synthetic generator
    /// (e.g. changing the tick anchor) actually takes effect on the
    /// next run instead of being silently ignored by INSERT OR IGNORE.
    /// Live ingest never touches `SYNTHETIC_TX_HASH` and is unaffected.
    pub async fn delete_synthetic_swaps_in_range(
        &self,
        pool_address: String,
        from_block: u64,
        to_block: u64,
        ...
        self.write(move |conn| {
            let n = conn.execute(
                ...
                params![pool, SYNTHETIC_TX_HASH, from, to],
            )?;
            Ok(n)
        })
        .await
